package com.weather.fileParser;
...
import com.weather.unzip.Unzip;

@Component
public class OpFileParser {
	...
	public List<String> readLinesAsListFromFile(String fileName) {
		...
		try {
			...
			throw new RuntimeException("error reading lines from file : " + e.getStackTrace());
		}
		return fileLines;
	}
	...
	public List<String> buildColumnForHeaderFromRowValuesList(String header, List<String> fileLines) {
		...
		if (StringUtils.isNotEmpty(header) && StringUtils.contains(headerLine, header)) {
			Map<String, List<String>> rowHeaderValuesMap = new HashMap<String, List<String>>();
